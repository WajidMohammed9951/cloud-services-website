      ...
      <div className="flex flex-col items-center justify-center max-w-screen-xl mx-auto h-full px-3 py-12">
        ...
        <div className="grid lg:grid-cols-3 gap-x-8 gap-y-16 px-4 pt-12 sm:pt-20 text-black">
          {supportLinks.map(({ id, title, subtitle, icon }) => (
            <div
              key={id}
              className="bg-gray-900 text-white rounded-xl shadow-lg shadow-green-700"
            >
              <div className="p-8">
                <div className="flex items-center justify-center w-16 h-16 p-4 bg-green-600 text-darkColor rounded-full mt-[-4rem]">
                  {icon}
                </div>
